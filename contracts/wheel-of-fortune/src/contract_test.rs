...
mod unit_tests {
    ...
    use crate::state::{
        AdminConfig, CoinReward, CollectionReward, TextReward, TokenReward, WheelReward,
        ADMIN_CONFIG, WHEEL_REWARDS,
    };
    ...
    fn instantiate_fail_with_invalid_maximum_spins() {
        ...
        match res {
            ContractError::CustomError { val } => {
                assert_eq!(
                    val,
                    "the maximum number of spins must be greater than 0".to_string()
                )
            }
            _ => panic!(),
        };
    ...
    fn instantiate_fail_with_invalid_random_seed() {
        ...
        let msg = InstantiateMsg {
            wheel_name: "test".to_string(),
            random_seed: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaah"
                .to_string(), // Invalid random seed
            max_spins_per_address: 1,
            is_public: true,
            is_advanced_randomness: false,
            nois_proxy: NOIS_PROXY.to_string(),
    ...
    fn add_whitelist_success() {
        ...
        let res = execute(
            deps.as_mut(),
            mock_env(),
            mock_info(CREATOR /*sender is creator*/, &[]),
            add_whitelist,
        )
        .unwrap();
        assert_eq!(
            res,
            Response::new().add_attribute("action", "add_whitelist")
        );
    }
    ...
    fn remove_whitelist_success_with_wheel_activated() {
        ...
        assert_eq!(
            res,
            Response::new().add_attribute("action", "remove_whitelist")
        );
    }
    ...
    fn add_coin_reward_success() {
        ...
        let res = execute(
            ...
            mock_info(
                CREATOR,
                &coins(10000u128, "uaura".to_string()), /* deposit funds */
            ),
            add_reward,
        )
        .unwrap();
    ...
    fn add_coin_reward_fail_with_long_label() {
        ...
        let res = execute(
            ...
            mock_info(
                CREATOR,
                &coins(100u128, "uaura".to_string()), /* deposit funds */
            ),
            add_reward,
        )
        .unwrap_err();
    ...
    fn add_coin_reward_fail_with_insufficent_fund() {
        ...
        let res = execute(
            ...
            mock_info(
                CREATOR,
                &coins(9999u128, "uaura".to_string()), /* insufficent funds, required 10000uaura */
            ),
            add_reward,
        )
        .unwrap_err();
    ...
    fn add_nfts_reward_success() {
        ...
        let transfer_msg: CosmosMsg = CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: test_address.to_string(),
            msg: to_json_binary(
                &CW721ExecuteMsg::<CW721Extension, CW721Extension>::TransferNft {
                    recipient: mock_env().contract.address.to_string(),
    ...
    fn add_reward_fail_with_too_many_slots() {
        ...
        WHEEL_REWARDS
            .save(deps.as_mut().storage, &(65536, wheel_rewards))
            .unwrap();
    ...
    fn add_reward_fail_with_too_many_rewards() {
        ...
        WHEEL_REWARDS
            .save(deps.as_mut().storage, &(u32::MAX, wheel_rewards))
            .unwrap();
    ...
    fn remove_reward_success() {
        ...
        WHEEL_REWARDS
            .save(deps.as_mut().storage, &(100, wheel_rewards))
            .unwrap();

        let slot = 0;
        let remove_reward = ExecuteMsg::RemoveReward { slot };
        ...
        assert_eq!(
            WHEEL_REWARDS.load(deps.as_mut().storage).unwrap(),
            (0, vec![])
        );
    }
    ...
    fn remove_reward_fail_with_unauthorized() {
        ...
        WHEEL_REWARDS
            .save(deps.as_mut().storage, &(1, wheel_rewards))
            .unwrap();
    ...
    fn activate_wheel_success() {
        ...
        assert_eq!(
            res,
            Response::new().add_attribute("action", "activate_wheel")
        );
    }
    ...
    fn spin_fail_with_spin_amount_not_enough() {
        ...
        let res = execute(
            ...
            mock_info(
                ...
                &[Coin {
                    denom: "uaura".to_string(),
                    amount: Uint128::from(999u128 /* Not enough fund */),
                }],
            ),
            spin_msg,
        )
    ...
    fn withdraw_reward_fail_with_wheel_not_activated() {
        ...
        let res =
            execute(deps.as_mut(), env, mock_info(CREATOR, &[]), withdraw_reward).unwrap_err();
        match res {
            ContractError::WheelNotActivated {} => {}
            _ => panic!(),
        }
